...
But what if we want to insert into a heap that is not indexed with 0? One solution is to be liberal and ``promote'' that heap so that after insertion it can store elements with any priorities. Remember the priority restriction can always be loosened but it cannot be tightened. However such a liberal approach might not be satisfactory. We might actually want to keep priority bounds as tight as possible. In that case we must begin by rewriting singleton function:
...
\end{code}
\noindent
This new function creates a heap that stores priority \texttt{p} and puts the bound \texttt{b} on resulting singleton heap. We must supply \texttt{singletonB} with a proof that putting restriction \textbf{b} on the heap storing priority \textbf{p} is possible. But if we try to write new \texttt{insertB} function we encounter problems:



But what if we actaully want to enforce bounds on a singleton heap? In that situation we must supply \texttt{singleton} function with a proof that the priority we are inserting into the singleton heap can really be insterted into it. To do this we will need a few additional functions. First of all we need a new singleton function that will construct a singleton Heap with a bound. This requires us to supply additional parameter that is evidence that priority we just inserted into our singleton heap is lower than the bound.
